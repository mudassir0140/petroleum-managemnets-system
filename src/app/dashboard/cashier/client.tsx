...
export function CashierOverviewClient({ session }: { session: CashierSession }) {
  const { activeShift, transactions, startShift, submitHandover } = useCashierShift(session.cashierId, session.pumpId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={`Welcome, ${session.cashierName}`} description={`Assigned to pump: ${session.pumpId}`} />
          ...
          <form className="space-y-4" onSubmit={handleHandover}>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Name of the next cashier"
                className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Optional notes for the handover"
                className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
              />
            </div>
            <button
