...
import { ClockIcon, GaugeIcon, TankIcon, TruckIcon, UsersIcon } from "@/components/icons";
import {
  managerIncomingTankers,
  managerPump,
  managerStaff,
  managerTanks,
} from "@/lib/dashboard/data/pump-manager-portal";
...
export default function PumpManagerOverviewPage() {
  const pump = managerPump();
  const staff = managerStaff();
  const tanks = managerTanks();
  const upcomingTankers = managerIncomingTankers().slice(0, 3);

  const staffPresent = staff.filter((s) => s.week[s.week.length - 1] === "P").length;

  return (
    <div className="space-y-6">
      <PageHeader title={pump.name} description={`${pump.address}, ${pump.city} — daily operations for your assigned pump only.`} />

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Today's revenue" value={formatCurrency(pumpTodayRevenue(pump))} icon={GaugeIcon} />
        <StatCard label="Today's liters" value={formatLiters(pumpTodayLiters(pump))} icon={TankIcon} />
        <StatCard label="Staff present today" value={`${staffPresent} / ${staff.length}`} icon={UsersIcon} />
        <StatCard label="Tankers inbound" value={String(upcomingTankers.length)} icon={TruckIcon} />
      </div>
      ...
      <SectionCard title="Quick links">
        <div className="grid grid-cols-2 gap-3 p-5 sm:grid-cols-4">
          {[
            { href: "/dashboard/pump-manager/staff", label: "Staff & Shifts", icon: UsersIcon },
            { href: "/dashboard/pump-manager/attendance", label: "Attendance", icon: ClockIcon },
            { href: "/dashboard/pump-manager/fuel-stock", label: "Fuel Stock", icon: TankIcon },
            { href: "/dashboard/pump-manager/tankers", label: "Tankers", icon: TruckIcon },
          ].map((link) => (
            <a
              key={link.href}
              href={link.href}
