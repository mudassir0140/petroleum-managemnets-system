...
export function AttendantOverviewClient({ session }: { session: AttendantSession }) {
  const { activeShift, sales, startShift, endShift } = useAttendantShift(session.attendantId, session.pumpId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Welcome, ${session.attendantName}`}
        description={`Assigned to pump: ${session.pumpId}`}
      />
