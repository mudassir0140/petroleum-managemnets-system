...
import { ATTENDANT_NAV_ITEMS } from "@/components/attendant/nav-items";
...
export function AttendantHeader({
  ...
  const currentLabel =
    ATTENDANT_NAV_ITEMS.find((item) => (item.match === "exact" ? pathname === item.href : pathname.startsWith(item.href)))?.label ?? "Dashboard";

  const initials = session.attendantName
    .split(" ")
    .map((p) => p[0])
    .slice(0, 2)
    .join("");
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border-subtle bg-surface-1/90 px-4 backdrop-blur sm:px-6">
      ...
      <div className="ml-auto flex items-center gap-3">
        <Badge tone={shiftActive ? "good" : "neutral"}>{shiftActive ? "Shift active" : "Not on shift"}</Badge>
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="hidden text-left sm:block">
              <span className="block text-xs font-medium text-ink-primary">{session.attendantName}</span>
              <span className="block text-[11px] text-ink-muted">Fuel Attendant</span>
            </span>
            ...
            <>
              ...
              <div className="absolute right-0 z-20 mt-2 w-64 rounded-xl border border-border-subtle bg-surface-1 p-2 shadow-lg">
                <div className="px-3 py-2">
                  <p className="text-sm font-medium text-ink-primary">{session.attendantName}</p>
                  <p className="truncate text-xs text-ink-muted">{session.attendantEmail}</p>
                </div>
                ...
                <div className="px-3 py-2 text-xs text-ink-muted">
                  ...
                  <p className="mt-0.5">
                    Role: <span className="font-medium text-ink-secondary">Fuel Attendant</span>
                  </p>
                </div>
                <div className="my-1 h-px bg-border-subtle" />
