...
export function SecurityGuardSidebar({
  session,
  pump,
  mobileOpen,
  onClose,
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2.5 px-5 pb-5 pt-6">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-ink-primary">{pump.name}</p>
          <p className="truncate text-xs text-ink-muted">{titleCase(session.assignedShift)} shift · {pump.id}</p>
        </div>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3">
        {SECURITY_GUARD_NAV_ITEMS.map((item) => {
      ...
      <div className="m-3 mt-2 flex items-start gap-2.5 rounded-xl bg-surface-3 p-3">
        ...
        <p className="text-[11px] leading-snug text-ink-muted">
          You can only monitor and log security activity for your own assigned pump,{" "}
          <span className="font-medium text-ink-secondary">{pump.name}</span>.
        </p>
      </div>
    </div>
  );
