    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2.5 px-5 pb-5 pt-6">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-ink-muted">{formatPumpAddress(pump)} · {pump.id}</p>
        </div>
      ...
      <div className="m-3 mt-2 flex items-start gap-2.5 rounded-xl bg-surface-3 p-3">
        ...
        <p className="text-[11px] leading-snug text-ink-muted">
          You&apos;re viewing data scoped to <span className="font-medium text-ink-secondary">{pump.name}</span> only. Other
          pumps&apos; records are never visible from this account.
        </p>
      </div>
    </div>
  );
