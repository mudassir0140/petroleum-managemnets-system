...
import { CASHIER_NAV_ITEMS } from "@/components/cashier/nav-items";
...
export function CashierHeader({
  ...
  const currentLabel =
    CASHIER_NAV_ITEMS.find((item) => (item.match === "exact" ? pathname === item.href : pathname.startsWith(item.href)))?.label ?? "Dashboard";

  const initials = session.cashierName
    .split(" ")
    .map((p) => p[0])
    .slice(0, 2)
    .join("");
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="hidden text-left sm:block">
              <span className="block text-xs font-medium text-ink-primary">{session.cashierName}</span>
              <span className="block text-[11px] text-ink-muted">Cashier</span>
            </span>
            ...
            <>
              ...
              <div className="absolute right-0 z-20 mt-2 w-64 rounded-xl border border-border-subtle bg-surface-1 p-2 shadow-lg">
                <div className="px-3 py-2">
                  <p className="text-sm font-medium text-ink-primary">{session.cashierName}</p>
                  <p className="truncate text-xs text-ink-muted">{session.cashierEmail}</p>
                </div>
                ...
                <div className="px-3 py-2 text-xs text-ink-muted">
                  ...
                  <p className="mt-0.5">
                    Role: <span className="font-medium text-ink-secondary">Cashier</span>
                  </p>
                </div>
                <div className="my-1 h-px bg-border-subtle" />
